  ...
  putLoadingScreen();
    ...
    onValue(DBref, (snapshot) => {
      ...
      for(var i = 0; i < keyList.length; i++){
        document.getElementById("main-content").innerHTML += `<div class="card mb-3" style="max-width: 90%;">
                                    <div class="row">
                                      <div class="col-md-4">
                                        <img src="`+ data[keyList[i]]["img"] +`" class="card-img" alt="..." style="height: 100%;">
                                      </div>
                                      <div class="col-md-4" style="width: 50%">
                                       <div style="margin: 20px; text-align: left; width: 100%;">
                                            <h3 class="mb-2 fw-bold" id="title">
                                                `+ data[keyList[i]]["title"] +`
                                            </h3>
                                            <p class="mb-2 fw-bold" id="desc">
                                                `+ data[keyList[i]]["desc"] +`
                                            </p>

                                             <p class="fw-bold" id="creator" style="color: #5A5A5A">
                                                Made by `+ data[keyList[i]]["creator"]  +`
                                            </p>
                                              <a id="enroll-a" href="./course_page.html?`+keyList[i]+`">
                                            <input type="button" style="padding: 10px;border: none; outline: none;  background: #1D90F5; border-radius: 5px;" value="Enroll" name="`+ keyList[i] +`" id="enroll">
                                            </a>
                                            </div>
                                      </div>
                                    </div>
